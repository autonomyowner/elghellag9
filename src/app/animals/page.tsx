...
export default function AnimalsListingsPage() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      if (searchTerm) {
        ...
        animalData = animalData.filter((animal: any) =>
  ...
  const getItemImage = (item: AnimalListing) => {
    if (item.images && item.images.length > 0) {
      return item.images[0];
    }
    return '/assets/sheep1.webp';
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5f3f0]">
      ...
      <UnifiedHero
        title="الحيوانات"
        subtitle="اكتشف أفضل المواشي والحيوانات من مزارع موثوقة"
        showSearch
        searchPlaceholder="ابحث عن أغنام، أبقار، ماعز..."
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="text-[#2d5016]">
              <span className="font-bold">{listings.length}</span> حيوان متاح
            </div>
            {user ? (
              <Link
                href="/animals/new"
