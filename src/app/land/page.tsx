...
export default function LandListingsPage() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      if (selectedCategory !== 'all') params.listingType = selectedCategory;
  ...
  const getItemImage = (item: LandListing) => {
    if (item.images && item.images.length > 0) {
      return item.images[0];
    }
    return '/assets/land01.jpg';
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5f3f0]">
      ...
      <UnifiedHero
        ...
        subtitle="اكتشف أفضل الأراضي الزراعية للبيع والإيجار"
        showSearch
        searchPlaceholder="ابحث عن أراضي زراعية..."
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="text-[#2d5016]">
              <span className="font-bold">{listings.length}</span> أرض متاحة
            </div>
            {user ? (
              <Link
                href="/land/new"
