...
const categories = [
  { id: 'all', label: 'جميع الشتلات' },
  { id: 'fruit_trees', label: 'أشجار مثمرة' },
  { id: 'ornamental', label: 'نباتات زينة' },
  { id: 'vegetables', label: 'خضروات' },
  { id: 'herbs', label: 'أعشاب' },
  { id: 'forest', label: 'أشجار حرجية' },
]
...
export default function NurseriesPage() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      if (searchTerm) {
        ...
        nurseryData = nurseryData.filter((item: any) =>
  ...
  const getItemImage = (item: NurseryListing) => {
    if (item.images && item.images.length > 0) {
      return item.images[0];
    }
    return '/assets/seedings01.jpg';
  };
  ...
  return (
    <div className="min-h-screen bg-[#f5f3f0]">
      ...
      <UnifiedHero
        ...
        subtitle="اكتشف أفضل الشتلات والنباتات من مشاتل موثوقة"
        showSearch
        searchPlaceholder="ابحث عن شتلات، أشجار، نباتات..."
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="text-[#2d5016]">
              <span className="font-bold">{listings.length}</span> شتلة متاحة
            </div>
            {user ? (
              <Link
                href="/nurseries/new"
